...
namespace Astro
{
   ...
   public sealed class OBB : BoundingVolume
   {
      ...
      {
          Vector2 exX = this.Axis[0] * this.Extents.X;
          Vector2 exY =  this.Axis[1] * this.Extents.Y ;

          Vector2 A,B,C,D;

          A.X =  this.Position.X + exX.X + exY.X;
          A.Y = this.Position.Y + exX.Y + exY.Y;

          B.X = this.Position.X - exX.X + exY.X;
          B.Y = this.Position.Y - exX.Y + exY.Y;

          C.X = this.Position.X - exX.X - exY.X;
          C.Y = this.Position.Y - exX.Y - exY.Y;

          D.X =  this.Position.X + exX.X - exY.X;
          D.Y = this.Position.Y + exX.Y - exY.Y;

          Lineas.Instancia.AddLine2D( Matrix.Identity, A, B , Color.Red );
          Lineas.Instancia.AddLine2D( Matrix.Identity, B,C, Color.Red );
          Lineas.Instancia.AddLine2D( Matrix.Identity, C, D, Color.Red );
          Lineas.Instancia.AddLine2D( Matrix.Identity,D,A, Color.Red );
