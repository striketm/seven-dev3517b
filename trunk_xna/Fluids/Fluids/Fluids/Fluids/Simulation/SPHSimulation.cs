   ...
   public sealed class SPHSimulation
   {
      ...
      private void CalculatePressureAndDensities(FluidParticles particles, IndexGrid grid)
      {
         ...
         foreach (var particle in particles)
         {
            ...
            {
               Vector2.Subtract(ref particle.Position, ref particles[nIdx].Position, out dist);
         ...
         for (int i = 0; i < particles.Count; i++)
         {
            ...
            foreach (var nIdx in grid.GetNeighbourIndex(particles[i]))
            {
               ...
               if (nIdx < i)
               {
                  ...
                  {
                     Vector2.Subtract(ref particles[i].Position, ref particles[nIdx].Position, out dist);
                     ...
                     Vector2.Multiply(ref f, scalar, out f);
                     particles[i].Force      -= f;
      ...
      private void CheckParticleDistance(FluidParticles particles, IndexGrid grid)
      {
         ...
         for (int i = 0; i < particles.Count; i++)
         {
            foreach (var nIdx in grid.GetNeighbourIndex(particles[i]))
            {
               Vector2.Subtract(ref particles[nIdx].Position, ref particles[i].Position, out dist);
               ...
               if (distLenSq < minDistSq)
               {
                  if (distLenSq > Constants.FLOAT_EPSILON)
                  {
                     ...
                     Vector2.Multiply(ref dist, 0.5f * (distLen - minDist) / distLen, out dist);
                     Vector2.Subtract(ref particles[nIdx].Position, ref dist, out particles[nIdx].Position);
                     Vector2.Subtract(ref particles[nIdx].PositionOld, ref dist, out particles[nIdx].PositionOld);
