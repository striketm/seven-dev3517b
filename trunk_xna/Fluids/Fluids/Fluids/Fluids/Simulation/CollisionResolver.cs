         ...
         foreach (var bv1 in this.BoundingVolumes)
         {
            foreach (var bv2 in this.BoundingVolumes)
            {
               if (bv1 != bv2)
               {
                  ...
                  {
                     hasCollided = true;
                     Vector2.Multiply(ref penetration, penLen, out penetration);
                     if (bv2.IsFixed)
                     {
      ...
      public bool Solve(FluidParticles particles)
      {
         ...
         foreach (var bv in this.BoundingVolumes)
         {
            foreach (var particle in particles)
            {
               ...
               {
                  hasCollided = true;
                  Vector2.Multiply(ref penNormal, penLen, out penetration);
                  ...
                  else
                  {
                     ...
                     Vector2.Subtract(ref particle.Position, ref particle.PositionOld, out v);
                     Vector2 tangent = penNormal.PerpendicularRight();
                     dp = Vector2.Dot(v, penNormal);
                     Vector2.Multiply(ref penNormal, dp * -this.Bounciness, out vn);
                     dp = Vector2.Dot(v, tangent);
                     Vector2.Multiply( ref tangent, dp * (1.0f - this.Friction), out vt );
                     Vector2.Add(ref vn, ref vt, out v);
                     particle.Position -= penetration;
                     Vector2.Subtract(ref particle.Position, ref v, out particle.PositionOld);
                  }
               }
            }
         }
